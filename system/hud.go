...
func (h *HUD) Draw(w donburi.World, screen *ebiten.Image) {
	...
	// Display current level in bottom right corner
	levelEntry := component.MustFindLevel(w)
	levelData := component.Level.Get(levelEntry)
	levelText := fmt.Sprintf("Level %d", levelData.LevelNumber)
	text.Draw(screen, levelText, assets.SmallFont, h.game.Settings.ScreenWidth-80, h.game.Settings.ScreenHeight-10, colornames.White)
}
