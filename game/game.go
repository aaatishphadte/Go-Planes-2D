...
func (g *Game) switchToGame(players []system.ChosenPlayer) {
	g.scene = scene.NewGame(players, g.screenWidth, g.screenHeight, g.switchToCredits)
}

func (g *Game) switchToCredits() {
	g.scene = scene.NewCredits(g.screenWidth, g.screenHeight, g.switchToTitle)
}
