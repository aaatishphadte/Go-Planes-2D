...
type Game struct {
	...
	screenWidth         int
	screenHeight        int
	switchToCreditsFunc func()
}

func NewGame(players []system.ChosenPlayer, screenWidth int, screenHeight int, switchToCreditsFunc func()) *Game {
	g := &Game{
		players:             players,
		level:               0,
		screenWidth:         screenWidth,
		screenHeight:        screenHeight,
		switchToCreditsFunc: switchToCreditsFunc,
	}
...
func (g *Game) createWorld(levelIndex int) donburi.World {
	...
	levelData := component.Level.Get(level)
	levelData.ProgressionTimer = engine.NewTimer(time.Second * 3)
	levelData.LevelNumber = levelIndex + 1 // 1-based for display
